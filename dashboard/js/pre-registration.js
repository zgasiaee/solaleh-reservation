$(function () {
    ...
    for (var index = year - 81 ; index <= year - 25 ; index++) {
      $('.year-parent').append('<span class="option number">' + index + '</span>')
    }
    ...
    $('#parent-select  label').click(function () {
      if ($(this).attr('id') === 'mother-select') {
        $(this).addClass('active-parent')
        $('#father-select').removeClass('active-parent')
      } else {
        $(this).addClass('active-parent')
        $('#mother-select').removeClass('active-parent')
      }
    })
